...
### Fixed
- **Channel Starter preview 404 JSON:** `/preview/{slug}` yang tidak ada di mesin `serve` (folder `sites/` gitignored; Simple Browser = localhost PC, bukan agen cloud) sekarang halaman HTML + tautan `examples/contoh-nexcent`. POST `/generate` 303 ke `/preview/{slug}` (bukan JSON `/sites/{slug}`). Portal proxy membaca Location `/preview/` atau `/sites/`.
