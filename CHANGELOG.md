...
### Fixed
- **Channel Starter preview 404 JSON:** `/preview/{slug}` yang tidak ada di mesin `serve` sekarang halaman HTML. POST `/generate` 303 ke `/preview/{slug}`. Demo git **`sites/contoh-nexcent`** (pengecualian gitignore) agar wizard **lama** yang hanya baca `sites/` tetap bisa setelah pull — `examples/` tidak dibaca proses lama, itu sebab Simple Browser masih JSON. `START-PREVIEW.bat` menghentikan listener 3010 lalu `serve`. Portal proxy membaca Location `/preview/` atau `/sites/`.
