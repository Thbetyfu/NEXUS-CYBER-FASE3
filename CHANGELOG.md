...
### Changed
- **Gitignore artefak lab:** JSON JobStore (`NEX-RED/jobs/data/*.json`, `JOB-*.json`, `*_scan.json`, `schedules.json`), PNG di akar repo, `.playwright-mcp/`, `.vercel/`. Kode Python Job + `immune_memory.json` + demo Channel Starter `sites/contoh-nexcent` **tetap** di Git. Duplikat blok Editor/OS–Python di `.gitignore` dihapus. Bukan perubahan WAF/Job.
