...
4. **Rp ~20rb/bulan** — website template Nexcent (4 palet) + domain lab `{slug}.nexus-lab.test` + header tepi Caddy + artefak `vercel.json`; **tanpa** Job Cowork / Loop, **tanpa** CNAME publik otomatis, **tanpa** klaim `*.vercel.app` sudah di belakang WAF gateway. Support unlimited tidak termasuk.
