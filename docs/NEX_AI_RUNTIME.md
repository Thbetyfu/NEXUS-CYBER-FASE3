...
Stack `deploy-local`: **tidak start** tanpa kedua nama di Ollama lokal. `START.bat` / `START-FOR-JURY.bat` memanggil `scripts/check_nex_ai.py` **sebelum** `docker compose up`. `START-OFFLINE.bat` ditolak (playground diarsip). Pesan operator: salin `nex_ai_q4_k_m.gguf` ke `nex-ai-models\` lalu `IMPORT-OLLAMA.bat` — bobot **bukan** unduhan Ollama Hub. Cek mandiri: `deploy-local/CHECK-NEX-AI.bat`.
