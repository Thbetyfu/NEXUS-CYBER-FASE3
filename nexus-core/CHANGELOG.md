...
- **Situs saya vs wizard lama:** `POST /sites/owned` pada FastAPI sebelum 60d91e0 = **405** (bukan daftar kosong). Portal memetakan 404/405 → **503** + pesan restart `cli.py serve`; operator loopback mendapat `operatorDetail`. Login juga `POST /sites/reassign` (formerGuestIds). Folder tanpa `portal_owner_*` tidak muncul sampai klaim slug eksplisit. Uji `channel-starter-owned.test.ts` + `test_preview_server.py`.
...
### Added
- **Klaim slug lama (Situs saya):** folder tanpa `portal_owner_*` bisa diikat ke sesi portal (tamu/akun) lewat **slug eksplisit** — `POST /api/channel-starter/sites/claim` → wizard `POST /sites/claim` (cap id/kind/email). **Bukan** assign semua folder disk ke yang login; **bukan** mencuri situs yang sudah dimiliki user lain (409); milik sendiri idempotent. Tanpa debit 20 Kr. Bukan WAF. Slug unik tidak berubah. Operator loopback `/operator/situs` daftar + attach (fail-closed, sama `/operator/tepi`). `GET /sites` publik tetap terkunci; Caddy tidak rewrite mutate. Auto-attach hanya jika debit Starter identitas itu memuat `slug=` (generate baru; folder pitch lama tetap harus diketik). Uji `test_ownership.py` + `channel-starter-owned.test.ts`.

- **Situs saya (sesi Channel Portal):** generate menulis `portal_owner_id` / `portal_owner_kind` / `portal_owner_email` (identitas cookie `nexus_portal_sid`, bukan email/WA usaha). `GET /situs` + `GET /api/channel-starter/sites` (sesi, **tanpa debit**) memanggil wizard loopback `POST /sites/owned` — bukan `GET /sites` publik, bukan Caddy/Next rewrite. User A tidak melihat situs user B. Folder tanpa owner tidak muncul sampai klaim slug atau generate baru. Preview tetap `GET /starter/preview/{slug}`. `*.vercel.app` jika sudah publish, else **belum publish**. Daftar HTML wizard `/preview` tidak lagi menumpuk semua klien. Bukan WAF Starter. Bukan SOC. Uji `test_ownership.py` + `portal-site-owner.test.ts`.

### Changed
- **Fill-starter `num_predict=512`:** `gemma3:1b` sering memotong JSON di 220 token (`done_reason=length`) sehingga parse gagal dua kali → `usedFallback: true`. Naikkan batas generate + prompt minta JSON lengkap/pendek. Bukan NEX-AI protect/reflex. Tidak debit 20 Kr. Uji `local-llm.test.ts`.
- **Fill-starter retry parse JSON:** generate Ollama yang kembali tapi slot tidak ter-parse (bukan hanya abort/timeout) di-retry **sekali**, timeout **35s** per percobaan (~70s batas). Percobaan kedua gagal → preset + `usedFallback: true` + `MSG_FILL_DOWN`. Bukan NEX-AI protect/reflex. Tidak debit 20 Kr. Vercel fail-closed. Uji `local-llm.test.ts`.
- **Fill-starter gemma3:1b lebih andal untuk demo:** timeout per percobaan **35s** (was 25s) + **satu retry** jika abort atau JSON tidak ter-parse (~70s batas, bukan tanpa batas). Prompt lebih pendek, `num_ctx=1024`, `num_predict=512`, `keep_alive=30m`. `START-LOCAL-LLM.bat` warmup loopback jika `:11434` sudah listen. Timeout/down/parse gagal → preset kategori + pesan jujur (`usedFallback: true`, bukan klaim model). Bukan NEX-AI protect/reflex. Tidak tunnel `:11434`. Vercel tetap fail-closed. Uji `local-llm.test.ts`.
