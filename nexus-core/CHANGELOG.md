...
- **Fill-starter gemma3:1b lebih andal untuk demo:** timeout per percobaan **35s** (was 25s) + **satu retry** jika abort atau JSON tidak ter-parse (~70s batas, bukan tanpa batas). Prompt lebih pendek, `num_ctx=1024`, `num_predict=220`, `keep_alive=30m`. `START-LOCAL-LLM.bat` warmup loopback jika `:11434` sudah listen. Timeout/down/parse gagal → preset kategori + pesan jujur (`usedFallback: true`, bukan klaim model). Bukan NEX-AI protect/reflex. Tidak tunnel `:11434`. Vercel tetap fail-closed. Uji `local-llm.test.ts`.
