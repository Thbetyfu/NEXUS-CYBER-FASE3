...
`python cli.py generate` men-deploy folder situs ke Vercel **jika** `VERCEL_TOKEN` atau `vercel login` ada di PC wizard (`../.env`, gitignore; restart `serve`). **Jangan** *Connect Git Repository* ke `NEXUS-CYBER-FASE3`.
