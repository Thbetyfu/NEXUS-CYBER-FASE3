...
| **Top-up Kredit (2026-09-01)** | **Bukan** PSP pihak ketiga (Midtrans, Stripe). **Isi** = pending + nomor WA `62895603358692` + form bukti (`proof_submitted`, `data/topup-proofs/`). Saldo **tidak** bertambah sampai operator `POST /api/kredit/topup/approve` atau UI localhost `/operator/topup`. QRIS/VA **belum live**. Keran lab sekunder. WhatsApp = instruksi bayar/bukti, **bukan** CTA beli paket UMKM |
