...
| **Top-up Kredit (2026-09-01)** | **Bukan** PSP pihak ketiga (Midtrans, Stripe). **Isi** = satu pending/`proof_submitted` per identitas (**409** jika sudah ada) + Nomor DANA (default WA `62895603358692`) + form bukti. Saldo **tidak** bertambah sampai operator `POST /api/kredit/topup/approve` atau UI localhost `/operator/topup`. QRIS/VA **belum live**. Keran lab sekunder. WhatsApp setelah Kirim bukti, **bukan** CTA beli paket UMKM |
