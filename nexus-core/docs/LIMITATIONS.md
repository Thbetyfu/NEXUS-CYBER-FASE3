...
19. **Model tulis Channel Portal** — Ollama `127.0.0.1:11434` + `GET /api/local-llm/health` + `POST /api/local-llm/fill-starter` (`gemma3:1b`). **Wajib sesi** cookie `nexus_portal_sid` (tamu setelah `/gate` atau akun) — tanpa sesi **401**, bukan 200 LLM. Rate limit tetap. Timeout ~35s + satu retry (abort atau JSON tidak ter-parse) → preset kategori + pesan jujur (`usedFallback`). URL bukan loopback ditolak. Preview copy = tagline/hero/tentang di `/pesan` (**Lihat teks**, tanpa debit). Debit 20 Kr hanya **Bayar**. **Jangan** tunnel `:11434`. NEX-AI protect/reflex tetap WAF, bukan copy situs.
