...
19. **Kredit** — ledger file **per tamu atau akun** di Channel Portal (`kredit-guest-{uuid}.json` / `kredit-account-{uuid}.json`); keran hanya `NEXUS_LEDGER_MODE=lab`. Tamu = cookie sesi `nexus_portal_sid` (bukan SSO). **Bukan** e-money, **bukan** settlement IDR, **bukan** jual Job 200 Kr otomatis. Top-up QRIS/VA milik pemilik + bukti + approve **disepakati, belum dikode**. **Bukan** Midtrans/Stripe. CLI `channel-starter` generate **tanpa** debit. Kasir Starter = `/pesan/umkm-starter` (alias `/order`).
---
