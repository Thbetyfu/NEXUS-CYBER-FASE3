...
4. **Rp ~20rb/bulan** — website template Nexcent (4 palet) + domain lab `{slug}.nexus-lab.test` + **header tepi** (Caddy `file_server` atau `vercel.json`: nosniff / frame DENY / Referrer-Policy / CSP) + publish Vercel **per folder** jika `VERCEL_TOKEN` di `channel-starter/.env` atau `vercel login` di PC wizard (bukan Connect Git monorepo Nexus; `--scope` hanya `CHANNEL_STARTER_VERCEL_SCOPE`); **tanpa** WAF Reflex, **tanpa** Job Cowork / Loop, **tanpa** CNAME publik otomatis, **tanpa** klaim `*.vercel.app` sudah di belakang WAF gateway. Nama usaha sama tidak menjamin slug sama — folder terpakai → `bu-grace-2` (dst.), bukan menimpa situs berbayar lama. Support unlimited tidak termasuk. Preview wizard `:3010` hanya di mesin yang `python cli.py serve`. Folder `sites/` gitignore kecuali demo `contoh-nexcent`. JSON JobStore (`NEX-RED/jobs/data/`, `JOB-*.json`) dan screenshot akar repo juga gitignore — bukan sumber kebenaran Git. JSON FastAPI `Site not found` atau `POST /sites/owned` / `POST /sites/claim` **405** hanya jika proses wizard **lama** masih hidup di port 3010 (kode baru: HTML + daftar milik sesi + klaim slug eksplisit). Restart `python cli.py serve`. Folder tanpa `portal_owner_*` tidak di-assign massal — klaim satu slug yang diketahui (atau operator loopback attach).
4aa. **Pagar tipis (35rb/28rb, `--tier tepi`)** — Reflex judi/deface di tepi **hanya** jika trafik masuk WAF `:8080` dengan Host di peta lab. Default **portfolio.nexus-lab.test** tetap. Upsell **menambah** `{slug}.nexus-lab.test` (bukan menimpa portfolio; bukan setiap generate Starter 20 Kr; **jangan klaim 100 UMKM / mass CNAME**). **Bukan** Job, **bukan** Loop di 20rb, **bukan** debit Starter 20 Kr, **bukan** pulih file Vercel, **bukan** `*.vercel.app` naked. Self-heal `INTEGRITY_MONITORED_DIR` tidak merestorasi origin Vercel.
4ab. **Portal Vercel** — etalase copy. Tidak mewajibkan Ollama `127.0.0.1:11434`. Generate/fill/operator di PC. Hostname named butuh zona Cloudflare pemilik (belum). Jangan tunnel `:11434` atau SOC.
4b. **Hosting tahap pilot** — PC operator 24/7 + **quick** Cloudflare tunnel (process keep-alive `KEEP-PORTAL-ALIVE`); **bukan** SLA data center, **bukan** hostname permanen. URL trycloudflare berubah jika `cloudflared` recreate. Named tunnel = beli/tambah domain di zona Cloudflare (belum). **524** = Node hung; **NXDOMAIN** = nama tunnel hilang. Control plane SOC **tidak** dipublikasikan lewat tunnel. Lihat [DISTRIBUTION_PILOT.md](./DISTRIBUTION_PILOT.md).
...
8. **Host map lab (bukan CNAME massal)** — satu instance gateway: `PROTECTED_HOST` (portfolio) + entri `NEXUS_HOST_MAP` / `nexus-host-map.json`. Entri junk (CRLF, wildcard, non-`nexus-lab.test`, origin non-http / bukan `channel-origin` atau `*.vercel.app`) **tidak** diikat/ditulis. Bukan provisioner 1000 UMKM. Job host-key mengikuti Active Workspace. Agen HTTP **dan** Playwright bind ke IP WAF + nama kanal (header `Host` / Chromium MAP) tanpa file hosts. PoW hotspot di named-host tanpa sesi tetap `sast_only` di alur browser. Publik tetap butuh DNS/tunnel agar *pengunjung* sampai ke WAF. **Origin lab:** `TARGET_BACKEND` = origin portfolio; slug tepi → `channel-origin:8099/{slug}/`. Onboard host *tambahan* persist. Di dalam container, `127.0.0.1:3001` bukan SOC host. Folder `playground/` tidak ada di repo — [PLAYGROUND_ARCHIVE.md](./PLAYGROUND_ARCHIVE.md).
...
19. **Model tulis Channel Portal** — Ollama `127.0.0.1:11434` + `GET /api/local-llm/health` + `POST /api/local-llm/fill-starter` (`gemma3:1b`). **Wajib sesi** cookie `nexus_portal_sid` (tamu setelah `/gate` atau akun) — tanpa sesi **401**, bukan 200 LLM. Rate limit tetap. Timeout ~35s + satu retry (abort atau JSON tidak ter-parse); `num_predict=512` agar JSON tidak terpotong. Gagal → preset kategori + pesan jujur (`usedFallback`). URL bukan loopback ditolak. Preview copy = tagline/hero/tentang di `/pesan` (**Lihat teks**, tanpa debit). Debit 20 Kr hanya **Bayar**. **Jangan** tunnel `:11434`. NEX-AI protect/reflex tetap WAF, bukan copy situs.
20. **Kredit** — ledger file **per tamu atau akun**. **Isi** menulis `data/kredit-topups.json` (pending / `proof_submitted`) — **tidak** menambah saldo. Form bukti + WhatsApp pemilik; berkas `data/topup-proofs/`. Kredit masuk via `POST /api/kredit/topup/approve` atau UI `/operator/topup` (loopback; Host publik / `cf-connecting-ip` → 404/403; identitas = email atau Tamu · ORDER, UUID hanya sekunder). QRIS/VA milik pemilik **belum live**. Keran `POST /api/kredit/faucet` hanya jika `NEXUS_LEDGER_MODE=lab` **dan** `NEXUS_LAB_FAUCET=1` (opt-in; default mati). Tamu = cookie `nexus_portal_sid`. **Bukan** e-money, **bukan** PSP, **bukan** jual Job 200 Kr otomatis. **Bukan** Midtrans/Stripe. CLI `channel-starter` generate **tanpa** debit. Kasir Starter = `/pesan/umkm-starter` (alias `/order`). Generate Node = `CHANNEL_STARTER_URL` loopback di PC; link preview = `CHANNEL_STARTER_PUBLIC_URL` atau **GET** `/starter` / `/starter/preview/{slug}` (Caddy/Next tidak mem-proxy mutate wizard). **Buka lagi situs sendiri:** `/situs` (sesi yang sama; tanpa debit). Situs generate sebelum field owner **tidak** muncul sampai generate ulang. Email/WA di form usaha **bukan** kunci milik. `GET /sites` FastAPI tetap loopback saja.
---
