...
19. **Model tulis Channel Portal** — Ollama `127.0.0.1:11434` + `GET /api/local-llm/health` + `POST /api/local-llm/fill-starter` (`gemma3:1b`). Timeout → preset kategori. URL bukan loopback ditolak. **Bukan** preview LLM. **Jangan** tunnel `:11434`. NEX-AI protect/reflex tetap WAF, bukan copy situs.
