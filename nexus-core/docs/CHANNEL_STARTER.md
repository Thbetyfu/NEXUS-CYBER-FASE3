...
| Portal kasir Starter | **Portal v0.1** — gerbang `/gate`; form pendek `/pesan/umkm-starter` (**Lihat teks** sesi portal, tanpa debit → **Bayar 20 Kr** fail-closed; fill `gemma3:1b` loopback, fallback preset). `/order` redirect. `/kredit` = pending + WA + form bukti. Approve `/operator/topup` (localhost). QRIS/VA **belum live**. Keran lab opt-in. Bukan PSP. Bukan CTA WA di kartu paket |
