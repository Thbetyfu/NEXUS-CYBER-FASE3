...
| Portal kasir Starter | **Portal v0.1** — gerbang `/gate`; form pendek `/pesan/umkm-starter` (**Lihat teks** sesi portal, tanpa debit → **Bayar 20 Kr** fail-closed; fill `gemma3:1b` loopback ~35s+satu retry (abort atau JSON tidak ter-parse), fallback preset jujur). `/order` redirect. `/kredit` = pending + WA + form bukti. Approve `/operator/topup` (localhost). QRIS/VA **belum live**. Keran lab opt-in. Bukan PSP. Bukan CTA WA di kartu paket |
...
**Vercel (PC operator):** simpan `VERCEL_TOKEN` di `channel-starter/.env` (gitignore). Restart `python cli.py serve` setelah mengubah token. Generate/`cli.py publish --slug` memakai token itu + `--token`; sesi `vercel login` **tidak** dipakai. Jangan Connect Git **NEXUS-CYBER-FASE3** ke project warung — tiap warung = `python cli.py publish --slug …`. Tanpa token: skip jujur (`publish gagal: set token di mesin wizard`). `--scope` hanya jika `CHANNEL_STARTER_VERCEL_SCOPE` diisi (slug team); jangan `VERCEL_ORG_ID` dari link project. Hosting `*.vercel.app` **bukan** WAF. Preview: portal `GET /starter/preview/{slug}`; pelanggan **`/situs`**. Mutate wizard hanya loopback `:3010` atau portal `POST /api/channel-starter/*`.
