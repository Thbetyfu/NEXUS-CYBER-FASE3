...
Channel Portal memakai **Ollama yang sama di loopback** untuk tulis copy Starter (`NEXUS_LOCAL_LLM_URL`, `NEXUS_LOCAL_LLM_MODEL=gemma3:1b`, `POST /api/local-llm/fill-starter` **dengan sesi portal**, tanpa debit Kredit). Generate Ollama memakai `keep_alive` + opsi kecil-model (`num_ctx` 1024); timeout ~35s per percobaan, satu retry jika abort atau JSON slot tidak ter-parse. Itu **bukan** substitusi `nex-ai-protect` / `nex-ai-reflex` di request path WAF. Jangan tunnel `:11434`.
