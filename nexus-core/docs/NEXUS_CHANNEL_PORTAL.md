...
/gate  (Login / Daftar / Tamu — skip jika cookie sesi ada; **wajib** untuk etalase)
 └─ /  (hub: pilih segmen)
     ...
     ├─ /situs       → situs generate sesi ini (bukan katalog lab)
     ├─ /sekolah     → sama
     ├─ /startup     → sama
...
**Header Shield 15rb/20rb** = **header tepi + hostname lab**, bukan WAF. **Edge Shield 35rb/28rb** (kartu portal; teknis `--tier tepi`) = Reflex judi/deface lewat WAF, **tambah** `{slug}.nexus-lab.test` ke host map (**`portfolio.nexus-lab.test` tetap**), **bukan** Job/Loop, **bukan** pulih Vercel, **bukan** `*.vercel.app` langsung, **bukan** setiap generate 20 Kr. Form `/pesan/*-tepi-*` tidak debit Starter. Lab: `python cli.py upsell enable --slug … --tier tepi` atau loopback `/operator/tepi`. Job/Loop = `/corporat` / `/cowork` / `--tier cowork`.

Preview HTML shareable untuk **setiap slug** (bukan hanya `bu-grace`): `GET /starter` dan `GET /starter/preview/{slug}` di `:3003` **tanpa** login. Caddy/Next **tidak** mem-proxy `POST /starter/generate`, `POST /publish`, `POST /upsell`, atau `GET /starter/sites` ke FastAPI. **Situs saya:** `/situs` (wajib cookie `nexus_portal_sid` yang sama dengan generate) → `GET /api/channel-starter/sites` → wizard `POST /sites/owned` (filter owner; tanpa WhatsApp orang lain; tanpa debit). Wizard yang belum di-restart setelah fitur owned/claim → **405** (bukan daftar kosong). Folder lama tanpa `portal_owner_*` **tidak** muncul otomatis — form **Klaim slug** di `/situs` (`POST /api/channel-starter/sites/claim`, tanpa debit) mengikat **satu** slug yang diketahui ke sesi ini (409 jika sudah milik orang lain; milik sendiri idempotent). Bukan assign seluruh disk. Operator loopback `/operator/situs` boleh daftar + attach (fail-closed, sama `/operator/tepi`). Login/daftar memanggil `POST /sites/reassign` untuk tamu yang jadi akun. `/gate` tetap wajib untuk hub/segmen/`/pesan`/`/kredit`/`/situs`. `/operator` dan approve Kredit **bukan** publik. `/starter/generate` dan `/starter/upsell` **bukan** path publik.

Form Starter: `/pesan/umkm-starter` (wajib: nama, WA usaha, kategori; cerita opsional). Tombol **Lihat teks** → `POST /api/local-llm/fill-starter` (**sesi** `nexus_portal_sid`, tanpa debit) menampilkan tagline/hero/tentang + sumber jujur (template kategori vs model lokal). Tanpa cookie → 401 / gerbang `/gate` (sama kelas dengan generate). Baru **Bayar 20 Kredit & buat site**. Gagal/timeout fill = template tetap tampil. Alamat, palet, dll. di **Lengkapi nanti**. Alias `/order`. Proxy generate (`Accept: application/json` atau `?format=json`; 303 → `POST /publish/{slug}`) ke `CHANNEL_STARTER_URL` loopback; preview HTML `/starter/preview/{slug}`. Checkout baru dengan nama tampilan sama **tidak** menimpa `sites/{slug}` — wizard mengalokasikan `bu-grace-2` (dst.) kecuali flag API `replaceExisting` (tanpa UI). Generate memanggil `deploy_manifest` → `publish_site` (sama `python cli.py publish --slug`; project Vercel = slug baru). UI sukses menampilkan URL Vercel atau **`publish gagal: vercel login di PC wizard, atau set VERCEL_TOKEN di channel-starter/.env`**. Retry: `POST /api/channel-starter/publish` `{ slug }` (sesi pelanggan, bukan publik). Token/`vercel login` **hanya di PC wizard**. **Jangan** Connect Git **NEXUS-CYBER-FASE3** ke project Vercel warung. `*.vercel.app` **bukan** WAF. Starter 20 Kr **bukan** Edge Shield; jangan upsell `--tier tepi` massal tiap slug. Slug hasil generate klien **tidak** ikut git; demo `sites/contoh-nexcent` ikut.
...
| Fill | Server: `POST /api/local-llm/fill-starter` `{ name, category, whatsapp, story }` → JSON slots (tagline, hero, about_body, cta_label, hours, description). **Sesi portal wajib** (cookie `nexus_portal_sid`; 401 `Sesi diperlukan` jika tidak). Rate limit. Bukan HTML. Timeout **~35s/percobaan**, **satu retry** jika abort atau JSON slot tidak ter-parse (~70s batas) → `{ usedFallback: true, error, …preset kategori }`. `keep_alive=30m`, `num_ctx=1024`, `num_predict=512`. URL bukan loopback → 503, tidak fetch. **Tidak** debit 20 Kr. Bukan NEX-AI WAF. |
