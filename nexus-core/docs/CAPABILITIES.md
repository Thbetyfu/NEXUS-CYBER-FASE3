...
Status mengikuti kode di `nexus-core-gateway`, `nexus-admin-dashboard`, dan `NEX-RED`. **Model produk:** [PRODUCT_MODEL.md](./PRODUCT_MODEL.md) (GaaS Edge Antibody Cowork). Pembaruan: 2026-09-03.
...
| --- | --- | --- |
| **Channel Starter** (form→template UMKM) | **Lab v0.1 + S-3/S-6** | Template Nexcent, 4 palet, Caddy header tepi; wizard `:3010` **loopback** untuk mutate; seed demo `sites/contoh-nexcent`; portal **GET** `/starter` + `/starter/preview/{slug}` **publik** (setiap slug; bukan proxy POST generate/publish/upsell atau GET `/sites`); **Situs saya** `/situs` (sesi, filter owner; klaim slug lama eksplisit tanpa debit); generate+publish Vercel lewat portal API (sesi+debit); slug unik per generate (`bu-grace-2` jika nama sama, kecuali `replaceExisting`); **Lihat teks** `POST /api/local-llm/fill-starter` (sesi, tanpa debit, `gemma3:1b` loopback) + token di PC wizard (bukan git FASE3; `*.vercel.app` bukan WAF) |
| **Pagar tipis** (tepi shared) | **Lab MVP** | Upsell `--tier tepi` → Caddy ke WAF `:8080` + Reflex; **host map** (portfolio + N slug). Generate 20 Kr **tidak** auto-join. **Bukan** mass CNAME; bukan `*.vercel.app` naked; bukan Job |
...
| Channel Portal / multi-tenant legacy | **Portal v0.1** | `nexus-gaas-web/` — login/daftar/tamu; **Kredit** Starter 20 Kr per identitas. Isi = pending + WA + form bukti; approve operator `/operator/topup` (localhost; kartu = email atau Tamu · ORDER, bukan UUID). QRIS/VA **belum live**. Keran lab opt-in. Bukan PSP; bukan F-10 |
