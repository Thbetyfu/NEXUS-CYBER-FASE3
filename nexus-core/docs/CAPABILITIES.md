...
| **Pagar tipis** (tepi shared) | **Lab MVP** | Upsell `--tier tepi` → Caddy ke WAF `:8080` + Reflex; **satu** slug/`PROTECTED_HOST` per lab. **Demo A (aktif):** tanpa upsell = **`portfolio.nexus-lab.test`**. Bukan Starter 20 Kr; bukan setiap warung; bukan pulih Vercel |
